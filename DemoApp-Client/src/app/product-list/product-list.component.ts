    ...
    <div class="p-6 max-w-7xl mx-auto space-y-6">
      ...
      >
        ...
        <div class="flex flex-wrap items-center gap-2">
          <span
            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200"
          >
      ...
      <div
        class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm"
      >
        <!-- Search Input -->
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @for (product of filteredProducts(); track product.id) {
          <div
            class="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition duration-200 flex flex-col justify-between overflow-hidden group"
          >
            <div class="p-5">
              <!-- Top Header & Category Tag -->
              <div class="flex items-start justify-between gap-2 mb-3">
                <span
                  class="inline-block px-2.5 py-0.5 rounded-md text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200"
                >
                  {{ product.category }}
                </span>
              ...
              <h3
                class="font-bold text-slate-800 text-base group-hover:text-indigo-600 transition-colors"
              >
                {{ product.name }}
              </h3>
            ...
            >
              ...
              @switch (product.status) {
                ...
                @case ('out-of-stock') {
                  <span
                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-rose-100 text-rose-800"
                  >
          ...
          <div
            class="col-span-full py-12 px-6 text-center bg-white rounded-xl border border-dashed border-slate-300"
          >
            <svg
  ...
  readonly products = signal<Product[]>([
    {
      ...
      status: 'in-stock',
      description:
        'Reduces wrist strain with natural ergonomic angle and precision optical sensor.',
    },
    {
      id: 'prod-102',
      sku: 'ELE-KB-002',
